...
class tpt_update_stock_move(osv.osv):
    ...
    _columns = {
        ...
        'issue_id': fields.char('Material Issue', size=1024),
...
#                         where move_id = %s and inventory_id in (select id from stock_inventory where name='TPT Update Stock Move')''',(move_ids[0],))
    ...
    def update_iss_one_by_one_780(self, cr, uid, ids, context=None):
        if context is None:
            context={}
        line = self.browse(cr, uid, ids[0])
        db_conn_string = "host='%s' port='%s' dbname='%s' user='%s' password='%s'"%(line.host, line.db_port, line.database, line.db_username, line.db_password)
        ...
        if line.issue_id:
            sql = '''
                select id,name,date_request,date_expec,department_id,request_type,state,gl_account_id,warehouse,dest_warehouse_id
                    from tpt_material_issue where state = 'done' and id = %s
            '''%(line.issue_id)
            cursor.execute(sql)
            issue_ids = cursor.fetchall()
            issue_obj = self.pool.get('tpt.material.issue')
...
#                             'dest_warehouse_id':issue[9],
                    ...
                    context.update({'create_issue_again':1})
                    new = issue_obj.create(cr, uid, vals, context)
                    self.map_iss_sm_one_by_one(cr, uid,new,context)
                    self.update_sm_inv_iss_one_by_one(cr, uid,new,context)
                    self.update_sm_iss_780(cr, uid,new,context)
        return self.write(cr, uid, ids, {'result':'TPT update issue 780 Done'})
    
    def map_iss_sm_one_by_one(self, cr, uid, issue_id, context=None):
        sql = '''
            select id from tpt_material_issue where id = %s
        '''%(issue_id)
        cr.execute(sql)
    ...
    def update_sm_inv_iss_one_by_one(self, cr, uid, issue_id, context=None):
        sql = '''
            select id from tpt_material_issue where id = %s
        '''%(issue_id)
        cr.execute(sql)
    ...
    def update_sm_iss_780(self, cr, uid, issue_id, context=None):
        sql = '''
            select id from stock_move where issue_id = %s
        '''%(issue_id)
        cr.execute(sql)
        
        for ma in cr.fetchall():
            move_obj = self.pool.get('stock.move')
            move_obj.write(cr, uid, [ma[0]],{'location_dest_id':23})
        
        cr.execute('''update tpt_material_issue set dest_warehouse_id = 23 where id = %s''',(issue_id,))
#         sql = '''
#             update tpt_material_issue set dest_warehouse_id = 23 where id = %s
#         '''%(issue_id)
#         cr.execute(sql)
