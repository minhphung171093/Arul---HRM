...
#         date = time.strftime('%Y-%m-%d'),
    ...
    def get_opening_balance(self):  
        ...
        sql = '''
            ...
            and move_id in (select move_id from account_voucher where state = 'posted' and (type_trans = 'payment' or type = 'payment') and (type_cash_bank = 'cash' or journal_id in (select id from account_journal where type = 'cash'))) 
            group by aml.date
        '''%(date_from)
        self.cr.execute(sql)
        ...
        sql = '''
            ...
            and move_id in (select move_id from account_voucher where state = 'posted' and (type_trans = 'receipt' or type = 'receipt') and (type_cash_bank = 'cash' or journal_id in (select id from account_journal where type = 'cash'))) 
            group by aml.date
        '''%(date_from)
        self.cr.execute(sql)
        for move in self.cr.dictfetchall():
    ...
    def get_move_ids(self):
        ...
        if type == 'payment':
            sql = '''
                    ...
                    union
                    select id from account_voucher where date between '%s' and '%s' and type_cash_bank = 'cash' and type_trans = 'payment' and state = 'posted'
            ...
            sql = '''
                    ...
                    union
                    select id from account_voucher where date between '%s' and '%s' and type_cash_bank = 'cash' and type_trans = 'receipt' and state = 'posted'
                '''%(date_from, date_to, date_from, date_to)
            self.cr.execute(sql)
            account_ids = [row[0] for row in self.cr.fetchall()]
            if account_ids:
            ...
            sql = '''
                    ...
                    union
                    select id from account_voucher where date between '%s' and '%s' and type_cash_bank = 'cash' and state = 'posted'
                '''%(date_from, date_to, date_from, date_to)
            self.cr.execute(sql)
            account_ids = [row[0] for row in self.cr.fetchall()]
            if account_ids:
