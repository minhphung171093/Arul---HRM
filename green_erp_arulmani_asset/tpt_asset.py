        ...
        for grn in self.browse(cr,uid,ids):
            if 'state' in vals and grn.type == 'in' and grn.state=='done':
                for grn_line in grn.move_lines:
                    print grn_line.product_id.name_template
                    if grn_line.product_id.cate_name=='assets':
                        ...
                        print "Hey! im Asset Product ya, you can drag me to Asset Register:)"
    ...
    def bt_load(self, cr, uid, ids,context=None):
        ...
        for asset in self.browse(cr, uid, ids):
            if asset.date_from:
                    ...
                    where ar.state='open' and dp.move_check='f' and dp.depreciation_date between '%s' and '%s'
                    '''%(asset.date_from, asset.date_to)
                    cr.execute(sql)
