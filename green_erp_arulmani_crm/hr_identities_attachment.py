...
class hr_identities_attachment(osv.osv):
    ...
    def _data_set(self, cr, uid, id, name, value, arg, context=None):
        ...
        if location:
            ...
            super(hr_identities_attachment, self).write(cr, SUPERUSER_ID, [id], {'store_fname': fname, 'file_size': file_size}, context=context)
        else:
            super(hr_identities_attachment, self).write(cr, SUPERUSER_ID, [id], {'db_datas': value, 'file_size': file_size}, context=context)
        return True

    _name = 'hr.identities.attachment'
    _columns = {
#         'name': fields.char('Attachment Name',size=256, required=True),
        'datas_fname': fields.char('File Name',size=256),
        'datas': fields.function(_data_get, fnct_inv=_data_set, string='Identity Copy', type="binary", nodrop=True),
        'store_fname': fields.char('Stored Filename', size=256),
        'db_datas': fields.binary('Database Data'),
        'file_size': fields.integer('File Size'),
        'name': fields.selection([('age','Age'),('address','Address'),('photo','Photo')],'Identity For'),
        'identity_given' : fields.char('Identity given', size=128),
        'identify_number' : fields.char('Identify Number', size=128),
