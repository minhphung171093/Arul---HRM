...
class bank_transfer(osv.osv_memory):
    _name = "bank.transfer"
    
    _columns = {
            'year': fields.selection([(num, str(num)) for num in range(2010, 2050)], 'Year', required = True),
    ...
    def print_report(self, cr, uid, ids, context=None):
        ...
        datas['model'] = 'bank.transfer'
        datas['form'] = self.read(cr, uid, ids)[0]
        datas['form'].update({'active_id':context.get('active_ids',False)})
        return {'type': 'ir.actions.report.xml', 'report_name': 'salary_bank_transfer_report', 'datas': datas}
bank_transfer()
