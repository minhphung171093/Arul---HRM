...
#         date = time.strftime('%Y-%m-%d'),
    ...
    def date_range(self):
        ...
        for single_date in rrule(DAILY, dtstart=date_from1, until=date_to1):
            if type == 'payment':
                account_ids = account_voucher_obj.search(self.cr,self.uid,[('date', '=', self.get_convert_date(single_date)), ('type_cash_bank', '=', 'cash'), ('type_trans', '=', 'payment')])
            elif type == 'receipt':
                account_ids = account_voucher_obj.search(self.cr,self.uid,[('date', '=', self.get_convert_date(single_date)), ('type_cash_bank', '=', 'cash'), ('type_trans', '=', 'receipt')])
            else:   
                account_ids = account_voucher_obj.search(self.cr,self.uid,[('date', '=', self.get_convert_date(single_date)), ('type_cash_bank', '=', 'cash')])
            if account_ids:
            ...
            if type == 'payment':
                ...
                if account_ids:
                    self.cr.execute('''
                        ...
                        aml.ref as ref, aml.name as voucher_desc
                ...
                if account_ids:
                    ...
                    return []
        else: # MAJOR ELSE Is Posted
            if type == 'payment':
                sql = '''
                        select id from account_voucher where date between '%s' and '%s' and type = 'payment' and journal_id in (select id from account_journal where type in ('cash','general')) and state = 'draft'
                    '''%(date_from, date_to)
                self.cr.execute(sql)
                account_ids = [row[0] for row in self.cr.fetchall()]
                if account_ids:
                ...
                sql = '''
                        select id from account_voucher where date between '%s' and '%s' and type = 'receipt' and journal_id in (select id from account_journal where type in ('cash','general')) and state = 'draft'
                    '''%(date_from, date_to)
                self.cr.execute(sql)
                account_ids = [row[0] for row in self.cr.fetchall()]
                if account_ids:
                ...
                sql = '''
                        select id from account_voucher where date between '%s' and '%s' and journal_id in (select id from account_journal where type in ('cash','general')) and state = 'draft'
                    '''%(date_from, date_to)
                self.cr.execute(sql)
                account_ids = [row[0] for row in self.cr.fetchall()]
                if account_ids:
