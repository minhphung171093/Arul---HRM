...
class Parser(report_sxw.rml_parse):
    ...
    def __init__(self, cr, uid, name, context):
        ...
        self.localcontext.update({
            ...
            # 'get_balance_30': self.get_balance_30,
            # 'get_balance_31_45': self.get_balance_31_45,
            # 'get_balance_46_60': self.get_balance_46_60,
            # 'get_balance_61_90': self.get_balance_61_90,
            # 'get_balance_over_90':self.get_balance_over_90,
            #===================================================================
    ...
    def get_customer(self):
        ...
        for line in bp_obj.browse(self.cr, self.uid, bp_ids):
                res.append({ 
                    ...
                    'name': line['name'] or '',  
                    'balance': self.get_balance(line['id'],line['customer_code'], date) or 0.00,
                    'balance_30':self.get_balance_30(line['id'],line['customer_code'],date) or 0.00,
                    'balance_31_45':self.get_balance_31_45(line['id'],line['customer_code'],date) or 0.00,
                    'balance_46_60': self.get_balance_46_60(line['id'],line['customer_code'],date) or 0.00,
                    'balance_61_90': self.get_balance_61_90(line['id'],line['customer_code'],date) or 0.00,
                    'balance_over_90':self.get_balance_over_90(line['id'],line['customer_code'],date) or 0.00,
...
# Modify sql logics on 03/10/2016 by P.vinothkumar
    def get_balance(self,id,code, date):
            credit = 0
#             sql  = '''
...
#                 '''%(code, date)
    ...
    def get_balance_30 (self,id,code, date):
            credit = 0
#             sql  = '''
...
#                 '''%(code,date,date)
    ...
    def get_balance_31_45 (self,id,code, date):
            credit = 0
#             sql  = '''
#                     select case when SUM(debit-credit)=0.00 or SUM(debit-credit) is null then 0.00 else SUM(debit-credit) end credit from account_move_line where 
#                     account_id=(select id from account_account where code = '0000'||'%s') and 
...
#                 '''%(code,date,date)
    ...
    def get_balance_46_60 (self,id,code,date):
            credit = 0
#             sql  = '''
#                     select case when SUM(debit-credit)=0.00 or SUM(debit-credit) is null then 0.00 else SUM(debit-credit) end credit from account_move_line where 
#                     account_id=(select id from account_account where code = '0000'||'%s') and 
...
#                 '''%(code,date,date)
    ...
    def get_balance_61_90 (self,id,code,date):
            credit = 0
#             sql  = '''
#                     select case when SUM(debit-credit)=0.00 or SUM(debit-credit) is null then 0.00 else SUM(debit-credit) end credit from account_move_line where 
#                     account_id=(select id from account_account where code = '0000'||'%s') and 
...
#                 '''%(code,date,date)
    ...
    def get_balance_over_90 (self,id,code,date):
            credit = 0
#             sql  = '''
#                     select case when SUM(debit-credit)=0.00 or SUM(debit-credit) is null then 0.00 else SUM(debit-credit) end credit from account_move_line where 
#                     account_id=(select id from account_account where code = '0000'||'%s') and 
